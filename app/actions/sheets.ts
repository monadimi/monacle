...
export async function createSheet(
  ...
  try {
    ...
    console.log(
      "Creating sheet via Admin payload:",
...
export async function updateSheet(
  id: string,
  updates: Partial<{ title: string; data: SheetData; is_shared: boolean }>,
) {
  try {
    const cookieStore = await cookies();
    const session = cookieStore.get("monacle_session");
    ...
    if (record.author !== user.id) {
      throw new Error("Forbidden: You do not own this sheet");
    }
    ...
    await pb.collection("sheets").update(id, data);
    return { success: true };
  } catch (error: unknown) {
