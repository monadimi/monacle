...
import { 
  Plus, Trash2, BarChart2, Table as TableIcon, 
  Settings, ChevronDown, ChevronRight, Save, Layout, Wand2, GripVertical, Share2
} from "lucide-react";
...
// --- Formula Engine ---
const evaluateFormula = (formula: string, row: Record<string, any>, allRows: Record<string, any>[], columns: SheetColumn[]) => {
  try {
    ...
    expression = expression.replace(refRegex, (match, colLabel) => {
      ...
      return isNaN(Number(val)) ? JSON.stringify(val) : val; 
    });
...
export default function SheetEditor({ initialData, currentUser, readOnly = false }: { initialData: any, currentUser: any, readOnly?: boolean }) {
  ...
  const [rows, setRows] = useState<Record<string, any>[]>(initialData.data.rows || []);
  ...
  // Debounced Save
  const debouncedSave = useCallback((newColumns: SheetColumn[], newRows: any[], newCharts: SheetChart[], newTitle: string) => {
    if (readOnly) return; 
                           ...
                           <tbody className="divide-y divide-slate-100">
                               {rows.map((row, idx) => (
                                   <tr key={row.id} className="group hover:bg-slate-50/50">
                                       ...
                                       {columns.map(col => {
                                           const isFormula = (typeof row[col.id] === 'string' && row[col.id].startsWith('='));
                                           ...
                                           <td key={`${row.id}-${col.id}`} className={cn("p-0 border-r border-b relative", isFormula ? "bg-indigo-50/20" : "")}>
                                               <input 
                                                 ...
                                                 onChange={(e) => updateCell(row.id, col.id, e.target.value)} // Don't force number parsing immediately
                                                 onBlur={(e) => {
                                                     // Try parse number if column is number AND not formula
                                                     if (col.type === 'number' && !e.target.value.startsWith('=')) {
                                                         const val = Number(e.target.value);
                                                         if (!isNaN(val)) updateCell(row.id, col.id, val);
                                                     }
                                                 }}
                                               />
                                               {isFormula && (
                               ...
                               <div className="flex-1 min-h-0">
                                   <ResponsiveContainer width="100%" height="100%">
                                       {chart.type === 'pie' ? (
                                           <PieChart>
                                               <Pie
                                                   data={rows.map(r => ({...r, ...Object.fromEntries(columns.map(c => {
                                                       if (typeof r[c.id] === 'string' && r[c.id].startsWith('=')) return [c.id, evaluateFormula(r[c.id], r, rows, columns)];
                                                       return [c.id, r[c.id]];
                                                   }))}))}
                                                   cx="50%"
                                                   cy="50%"
                                       ...
                                       <BarChart data={rows.map(r => ({...r, ...Object.fromEntries(columns.map(c => {
                                           if (typeof r[c.id] === 'string' && r[c.id].startsWith('=')) return [c.id, evaluateFormula(r[c.id], r, rows, columns)];
                                           return [c.id, r[c.id]];
                                       }))}))}> 
